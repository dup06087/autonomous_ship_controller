...
class ICPHandler:
    def __init__(self, main_instance, imu_corrector):
        ...
        self.device = o3d.core.Device("CPU:0")  # CPU 장치를 사용하도록 설정
    ...
    def lidar_callback(self, data):
        ...
        # print("lidar data type : ", type(data))
        ...
        try:
            ...
            if self.prev_scan is not None and self.main_instance.flag_icp_execute:
                ...
                # print("initial guess : ", self.icp_initial_guess)
                ...
                if reg_gicp.fitness > 0.8:
                    ...
                    # print("heading change : ", icp_heading_change)
                    icp_heading_change = np.trunc(icp_heading_change * 10) / 10
                    ...
                    velocity = round(math.sqrt(v_x**2+v_y**2), 2)
                    
                    self.icp_value_ready= True
                    self.current_value = {"latitude" : lat, "longitude" : lon, "heading" : current_heading, "pitch" : 0, "COG" : None, "velocity" : velocity, "forward_velocity" : round(v_x, 3), "rotational_velocity" : rotational_velocity}
                    self.processed_time = current_time
