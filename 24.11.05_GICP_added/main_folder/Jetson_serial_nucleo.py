            ...
            try:
                ...
                with self.lock_send:
                    if not self.transmit_queue.empty():
                        ...
                        # 시리얼 포트가 쓰기 준비가 되었는지 확인
                        if self.serial_port.writable():
                            self.serial_port.write(self.nucleo_sended_data)
                            self.flag_nucleo_alive["send"] = True
                        else:
                            print("Serial port not ready for writing. Retrying...")
        ...
        while self.running:
            ...
            if current_time - last_send_time >= 0.2:
                try:
                    ...
                    # print("sended data : ", sending_data)
