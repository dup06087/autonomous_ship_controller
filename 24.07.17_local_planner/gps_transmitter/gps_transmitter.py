...
# gps_data_file = "./24.07.17_local_planner/gps_transmitter/test_rotational_added.txt"
gps_data_file = "./24.07.17_local_planner/gps_transmitter/test_rotational_gnss_denied.txt"
...
try:
    with open(gps_data_file, 'r') as file:
        while True:
            try:
                # 파일에서 두 줄씩 읽기```
                lines = [next(file).strip(), next(file).strip(), next(file).strip(), next(file).strip()]
