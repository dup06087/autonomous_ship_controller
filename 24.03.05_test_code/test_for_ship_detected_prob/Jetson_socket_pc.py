...
class Server_pc:
    def __init__(self, boat_instance, receive_coeff_port= 5000, receive_port=5003, send_port=5004, send_obstacle_port=5005):
        ...
        self.pc_coeff = {"coeff_kf" : 3.5, "coeff_kd" : 0.6, "voxel_size" : 0.05, "intensity" : 30, "dbscan_eps" : 0.1 , "dbscan_minpoints" : 5, "coeff_vff_repulsive_force" : 0.2}
        self.flag_socket_pc = [False, False, False, False] # 4 > coeff
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as receive_socket, \
                ...
                while True:
                    try:
                        ...
                        print("Waiting for receive coeff connection...")
                        continue
