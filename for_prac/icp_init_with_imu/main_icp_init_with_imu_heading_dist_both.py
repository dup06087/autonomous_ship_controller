...
class IMUCorrector:
    def __init__(self):
        ...
        self.acc_list = []  # To store accelerations between ICP calls
    ...
    def imu_callback(self, data):
        ...
        # Extract linear acceleration and correct it
        linear_acceleration = data.linear_acceleration
        roll, pitch, yaw = self.quaternion_to_euler(data.orientation.x, data.orientation.y, data.orientation.z, data.orientation.w)
        corrected_acc = self.rotate_acceleration(linear_acceleration.x, linear_acceleration.y, linear_acceleration.z, roll, pitch, yaw)

        # Store the corrected acceleration for post-ICP processing
        self.acc_list.append((corrected_acc, delta_time, current_heading))
    ...
    def process_acc_after_icp(self, initial_vx, initial_vy):
        """
        Process stored accelerations after ICP to update velocity and calculate dnorth, deast via dead reckoning.
        
        Arguments:
        initial_vx -- The initial velocity in the x-direction (from ICP)
        initial_vy -- The initial velocity in the y-direction (from ICP)

        Returns:
        accumulated_vx -- The final velocity in the x-direction (after processing IMU data)
        accumulated_vy -- The final velocity in the y-direction (after processing IMU data)
        dnorth -- The total distance traveled in the north direction (global frame)
        deast -- The total distance traveled in the east direction (global frame)
        """
        accumulated_vx = initial_vx
        accumulated_vy = initial_vy
        dnorth = 0
        deast = 0
        
        for acc_data, delta_time, heading in self.acc_list:
            heading_rad = math.radians(heading-90)
            
            # Update velocity with stored acceleration (linear motion model)
            local_vx = acc_data[0] * delta_time
            local_vy = acc_data[1] * delta_time
            
            # Transform to global frame using heading
            global_v_north = local_vx * math.cos(heading_rad) - local_vy * math.sin(heading_rad)
            global_v_east = local_vx * math.sin(heading_rad) + local_vy * math.cos(heading_rad)
            
            # Accumulate the velocities
            accumulated_vx += global_v_north
            accumulated_vy += global_v_east
            
            # Dead reckoning: Calculate change in position (dnorth, deast) over time using velocities
            dnorth += accumulated_vx * delta_time
            deast += accumulated_vy * delta_time

        # Clear the acceleration list after processing
        self.acc_list.clear()

        # Return final accumulated velocity and dead-reckoning based position
        return accumulated_vx, accumulated_vy, dnorth, deast
        ...
        try:
            ...
            if self.prev_scan is not None:
                ...
                if reg_gicp.fitness > 0.8:
                    ...
                    # Process accumulated IMU accelerations
                    final_vx, final_vy, self.dnorth, self.deast = self.imu_corrector.process_acc_after_icp(v_x, v_y)

                    # Update IMU with the combined velocities
                    self.imu_corrector.post_icp_reset(final_vx, final_vy)
                    print("vx vy : ", final_vx, final_vy)
