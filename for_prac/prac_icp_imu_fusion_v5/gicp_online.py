...
class ICPHandler:
    def __init__(self, gnss_handler, experiment_folder):
        ...
        self.log_file = os.path.join(self.experiment_folder, "icp_log.txt")
    ...
    def remove_ship_body(self, pcd, ship_body_bounds):
        # GPU에서 CPU로 데이터를 이동
        pcd_tensor = pcd.point.positions.to(o3d.Device("CPU:0"))
        ...
        filtered_pcd = o3d.t.geometry.PointCloud(o3c.Tensor(filtered_pcd_tensor, dtype=o3c.float32, device=o3d.Device("CUDA:0")))
        return filtered_pcd
    ...
    def save_current_log_callback(self, lat, lon, heading):
        try:         
            log_list = {"lat" : lat, "lon" : lon, "heading" : heading}
    ...
    def lidar_callback(self, data):
        ...
        try:
            ...
            min_bound = np.array([-35, -35, -0.5])
            max_bound = np.array([35, 35, 2])
            ...
            if self.prev_scan is not None:
                ...
                self.icp_initial_guess = np.array([[np.cos(theta), -np.sin(theta), 0, 0],
                ...
                reg_gicp = o3d.pipelines.registration.registration_generalized_icp(
                        ...
                        o3d.pipelines.registration.ICPConvergenceCriteria(relative_fitness=1e-6, relative_rmse=1e-6, max_iteration=10)
                        # callback_after_iteration=self.icp_callback
                )
                
                transf = reg_gicp.transformation  # Convert the result back to a numpy array
                ...
                if reg_gicp.fitness > 0.8:
                    ...
                    self.save_current_log_callback(lat, lon, current_heading)
    ...
    def crop_roi(self, cloud, min_bound, max_bound):
        min_bound_tensor = o3d.Tensor(min_bound, dtype=o3d.float32, device=cloud.point.positions.device)
        max_bound_tensor = o3d.Tensor(max_bound, dtype=o3d.float32, device=cloud.point.positions.device)
