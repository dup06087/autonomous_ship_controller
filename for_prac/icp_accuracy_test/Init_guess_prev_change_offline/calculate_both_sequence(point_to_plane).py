...
def main_icp(pcd_file1, pcd_file2, prev_dx=0, prev_dy=0, prev_dyaw=0, min_height=0):
    ...
    }, delta_x, delta_y, radians(yaw_change)
...
def process_folder_for_icp_and_gnss(folder_path, gnss_file):
    ...
    results = []
    prev_dx, prev_dy, prev_dyaw = 0, 0, 0  # Initialize previous deltas to zero for the first iteration
    ...
    for i in range(len(pcd_files) - 1):
        ...
        if abs(time2 - time1) <= 0.21:
            ...
            # Run ICP registration and measure time, passing the previous deltas for initial guess
            icp_result, prev_dx, prev_dy, prev_dyaw = main_icp(pcd_file1, pcd_file2, prev_dx, prev_dy, prev_dyaw)
    ...
    if results:
        save_results_to_txt(results, output_file="icp_gnss_results_point_to_plane.txt")
        print("Results saved to icp_gnss_results_point_to_plane.txt")  # Debug print
    else:
