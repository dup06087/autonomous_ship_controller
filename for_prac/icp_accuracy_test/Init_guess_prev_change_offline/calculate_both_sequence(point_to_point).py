...
# Update to use previous deltas
def main_icp(pcd_file1, pcd_file2, prev_dx=0, prev_dy=0, prev_dyaw=0, min_height=0, icp_type='point-to-plane'):
    ...
    # Use previous delta for initial guess
    icp_initial_guess = np.array([
        [np.cos(radians(prev_dyaw)), -np.sin(radians(prev_dyaw)), 0, prev_dx],
        [np.sin(radians(prev_dyaw)),  np.cos(radians(prev_dyaw)), 0, prev_dy],
        [0,                          0,                         1, 0],
        [0,                          0,                         0, 1]
    ])
    ...
    return {
        ...
        "calculated_time": icp_time
    }, delta_x, delta_y, yaw_change
...
def process_folder_for_icp_and_gnss(folder_path, gnss_file, icp_type='point-to-plane'):
    ...
    results = []
    prev_dx, prev_dy, prev_dyaw = 0, 0, 0  # Initialize previous deltas
    ...
    for i in range(len(pcd_files) - 1):
        ...
        if abs(time2 - time1) <= 0.21:
            ...
            # Run ICP registration with previous transformation deltas
            icp_result, prev_dx, prev_dy, prev_dyaw = main_icp(pcd_file1, pcd_file2, prev_dx, prev_dy, prev_dyaw, min_height=0, icp_type=icp_type)
    ...
    if results:
        save_results_to_txt(results, output_file=f"icp_gnss_results_{icp_type}.txt")
        print(f"Results saved to icp_gnss_results_{icp_type}.txt")  # Debug print
    else:
...
if __name__ == "__main__":
    ...
    icp_type = 'point_to)point'  # 또는 'point-to-plane'
